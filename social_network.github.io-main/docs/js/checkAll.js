 ...
 export function checkUsername() {
  ...
  submitButton.addEventListener('click', (e) => {
    ...
    if(checkPassword() && checkPhoneandEmail() && checkUsername()  && checkGender() && checkRegister()) {
     currentUser[0] = new User(username, gender, email_or_phone, password)

     
     submitButton.disabled = false
     currentUser[0].generationAvatar()
     img.style.display = "none"
     userData.push(currentUser[0])
     console.log(userData);
     registerCompleteText.innerHTML = `User ${currentUser[0].name}, with ID ${currentUser[0].id} registered`
